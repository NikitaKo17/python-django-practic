...
export-env {
    ...
    let virtual_env = r#'C:\WorkFolder\python-django-practic\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'python-django-practic'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'python-django-practic'#
    }
    ...
    if (has-env 'TCL_LIBRARY')  {
        let $new_env = $new_env | insert TCL_LIBRARY r#''#
    }
    if (has-env 'TK_LIBRARY')  {
        let $new_env = $new_env | insert TK_LIBRARY r#''#
    }
